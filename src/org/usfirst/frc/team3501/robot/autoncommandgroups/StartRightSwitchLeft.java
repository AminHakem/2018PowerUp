...
public class StartRightSwitchLeft extends CommandGroup {
  ...
  public StartRightSwitchLeft() {
    addSequential(new MoveToTarget(48, 5)); // did 48 in so it leaves margin for
                                            // how much distance the cube is
                                            // dropping
    // robot drives straight 240 in
    addSequential(new DriveForward(
        RIGHT_BACK_WALL_TO_SWITCH_CUBES * Constants.Auton.SCALE_FACTOR_TESTING,
        9.5));
    ...
    addSequential(new DriveForward(
        RIGHT_WALL_TO_LEFT_WALL_HORIZONTAL
    ...
    addSequential(new DriveForward(
        LEFT_WALL_TO_LEFT_SCALE_VERTICAL * Constants.Auton.SCALE_FACTOR_TESTING,
        0));
    ...
    addSequential(
        new DriveForward(
            LEFT_SCALE_TO_SCALE * Constants.Auton.SCALE_FACTOR_TESTING, 1.5));
    MathLib.debugPrint("Finished command 6");
