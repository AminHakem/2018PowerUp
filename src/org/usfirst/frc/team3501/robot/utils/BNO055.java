 ...
 *
 * To use the sensor, wire up to it over I2C on the roboRIO. Creating an instance of this class will
 * cause communications with the sensor to being.All communications with the sensor occur in a
 * separate thread from your robot code to avoid blocking the main robot program execution.
 *
 * Example: private static BNO055 imu;
 *
 * public Robot() { imu = BNO055.getInstance(BNO055.opmode_t.OPERATION_MODE_IMUPLUS,
 * BNO055.vector_type_t.VECTOR_EULER); }
 *
 * You can check the status of the sensor by using the following methods: isSensorPresent();
 ...
 *
 * Once the sensor calibration is complete , you can get position data by by using the getVector()
 * method. See this method definiton for usage info.
 *
 * This code was originally ported from arduino source developed by Adafruit. See the original
...
public class BNO055 {
  ...
  /**
   * Get an instance of the IMU object plugged into the onboard I2C header. Using the default address
   * (0x28)
   *
   * @param mode the operating mode to run the sensor in.
  ...
  public SystemStatus getSystemStatus() {
     ...
     * = Initializing Peripherals 3 = System Initalization 4 = Executing Self-Test 5 = Sensor fusion
     * algorithm running 6 = System running without fusion algorithms
     */

    status.system_status = read8(reg_t.BNO055_SYS_STAT_ADDR);

    /*
     * Self Test Results (see section ) -------------------------------- 1 = test passed, 0 = test
     * failed
     *
     * Bit 0 = Accelerometer self test Bit 1 = Magnetometer self test Bit 2 = Gyroscope self test Bit 3
     * = MCU self test
     *
     * 0x0F = all good!
     */
     ...
     * initialization error 2 = System initialization error 3 = Self test result failed 4 = Register map
     * value out of range 5 = Register map address out of range 6 = Register map write error 7 = BNO low
     * power mode not available for selected operation mode 8 = Accelerometer power mode not available 9
     * = Fusion algorithm configuration error A = Sensor configuration error
     */
   ...
   * period the sensor will not return position vector data. Once initialization is complete, data can
   * be read, although the sensor may not have completed calibration. See isCalibrated.
   *
  ...
  /**
   * Gets a vector representing the sensors position (heading, roll, pitch). heading: 0 to 360 degrees
   * roll: -90 to +90 degrees pitch: -180 to +180 degrees
   *
   ...
   *
   * Operating Mode Data Output Rate IMU 100 Hz COMPASS 20 Hz M4G 50 Hz NDOF_FMC_OFF 100 Hz NDOF 100
   * Hz
   *
  ...
  /**
   * The heading of the sensor (x axis) in continuous format relative to the initial heading of the
   * robot. Eg rotating the sensor clockwise two full rotations will return a value of 720 degrees.
   * The getVector method will return heading in a constrained 0 - 360 deg format if required.
   *
